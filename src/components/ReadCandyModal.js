...
const ReadCandyModal = ({ isOpen, onClose, senderName, message }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      ...
      >
        <div className="flex flex-col w-64 text-center items-center justify-center content-center">
        {/* 발신자 이름 (항상 하단 고정) */}
        ...
        {/* 메시지 내용 (왼쪽 정렬 & 중앙 배치) */}
          <div className="flex-1 flex w-64 h-80">
            <div className="text-lg w-full py-5 px-2 overflow-y-auto h-80">
              {message}
            </div>
          </div>
        ...
        <button
          onClick={onClose}
          className=" w-full h-12 content-center mt-4 bg-pink-200 text-amber-950 flex justify-center items-center rounded-lg font-bold text-center shadow-gray-400 shadow-md"
        >
          닫기
        </button>
